...
PM2 Process Manager &​ Auto-Boot Persistence
Recommended: 4GB Swap Setup
Recommended: UFW Firewall Configuration
...
5.2 Create Database & User (The 2-Line Simple Way)
Instead of complicated schema grants, make the app user a ​SUPERUSER​. This gives full permissions automatically and eliminates all permission errors:

sudo -u postgres psql
Inside ​psql​ console, run just these 2 lines:

CREATE USER my_user WITH SUPERUSER PASSWORD 'MySecurePassword123!';
CREATE DATABASE my_database OWNER my_user;
\q
(Or as a single bash command without entering psql):

sudo -u postgres psql -c "CREATE USER my_user WITH SUPERUSER PASSWORD 'MySecurePassword123!'; CREATE DATABASE my_database OWNER my_user;"
...
Swap memory prevents unexpected process kills (​OOM Killer​) when compiling code or running databases on smaller VPS instances (≤​ 4GB RAM).
